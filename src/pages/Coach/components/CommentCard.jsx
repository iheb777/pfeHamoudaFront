...
const CommentCard = ({ member }) => {
  return (
    <Container>
      <Wrapper>
        <Avatar sx={{width: '34px', height: '34px'}} src={member.userId.img} >{member.userId.name.charAt(0)}</Avatar>
        <Details>
          <Name>{member.text}</Name>
          <EmailId>{member.userId.email}</EmailId>
        </Details>
        <Role
      ...
      <Access>{member.userId.role}</Access>
    </Container>
  );
};
